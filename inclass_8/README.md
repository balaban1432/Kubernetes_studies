...
# şimdi kube-worker a bir taint daha ekleyelim ama effect=NoExecute olsun.
ikinci shell --> kubectl get po -o wide
kubectl taint nodes kube-worker color=blue:NoExecute
...
```bash
kubectl taint nodes kube-worker clarus=way:NoSchedule-
kubectl taint nodes kube-worker color-
```